...
rule make_filtered_tracks:
    ...
    shell: 
        ...
        bam_output={output}/${{file_prefix}}_peak_filtered_minus.bam
        samtools view -b -h -L $ref {input.minus} > $bam_output  2>> {log}
        samtools index $bam_output &>> {log}
        bamCoverage -b $bam_output -o ${{bam_output%.bam}}.bw -p 36 -v --binSize 1 &>> {log}

        # Plus

        bam_output={output}/${{file_prefix}}_peak_filtered_plus.bam
