...
rule polyAreads_filterbam_byreadnames:
    ...
    shell:  
        ...
        picard -Xmx50g FilterSamReads -I {input.plus} -O {output.bam_plus} -READ_LIST_FILE {output.readname_plus} -FILTER includeReadList -MAX_RECORDS_IN_RAM 5000000 &>> {log}
        ...
        echo Filter sam reads by read names... &>> {log}
        picard -Xmx50g FilterSamReads -I {input.minus} -O {output.bam_minus} -READ_LIST_FILE {output.readname_minus} -FILTER includeReadList -MAX_RECORDS_IN_RAM 5000000 &>> {log}
...
rule bam_coverage_all_filtered_reads:
    ...
    output:
        bw_plus="outputs/2c1_alignment_signals/all_filtered_reads/dedup_uniq_genomicAfiltered_merged_plus_ext.bw",
        bw_minus="outputs/2c1_alignment_signals/all_filtered_reads/dedup_uniq_genomicAfiltered_merged_minus_ext.bw",
...
rule bam_coverage_polyA_reads:
    ...
    params:
        ...
        ncores=config["ncores"],
    output:
        bw_plus="outputs/2c1_alignment_signals/polyA_reads/polyAreads_beforegenomicAfiltering_plus_nowrongstrand_ext.bw",
        bw_minus="outputs/2c1_alignment_signals/polyA_reads/polyAreads_beforegenomicAfiltering_minus_nowrongstrand_ext.bw",
